...
💡 建议：您可以截图您的起始页界面，并将图片上传到仓库，替换此处的文本以展示效果。
...
本地安装了 Node.js (仅用于初始化数据库，后续维护不需要)。
...
Framework preset: 选 None (或者 Nuxt/Vue 也可以，因为本项目是纯静态+Functions)
...
点击 Create，创建一个新数据库，建议命名为 linkbox-db。

回到刚才创建的 Pages 项目 -> Settings -> Functions。

找到 D1 Database Bindings，点击 Add binding：

Variable name: DB (注意：必须是大写的 DB)

D1 database: 选择刚才创建的 linkbox-db

重新部署：绑定完成后，请务必去 Deployments 页面，点击 Create deployment -> Retry deployment (或者手动在 GitHub 触发一次提交)，让数据库绑定生效。

第四步：初始化数据库 (关键)

为了让程序能运行，你需要初始化数据库表结构。请选择以下任意一种方法：
...
# 3. 初始化表结构 (请将 <DATABASE_ID> 替换为你 D1 数据库的实际 ID)
# 可以在 Cloudflare D1 详情页找到 ID
wrangler d1 execute linkbox-db --remote --file=./db/schema.sql

# 4. 初始化密码配置表 (默认密码设为 admin)
...
进入 Cloudflare D1 数据库详情页，点击 Console 标签。

复制项目里 db/schema.sql 文件的完整内容，粘贴到控制台并点击 Execute。

清除控制台，再复制以下 SQL 语句并 Execute 以初始化密码：
